...
-- Find the full record of the app that has the most reviews 
...
--Find the average rating for each category ordered 
--      by the highest rated to lowest rated.
SELECT category, AVG(rating) FROM analytics 
  GROUP BY category 
  ORDER BY avg DESC;

--Find the name, price, and rating of the most 
...
--Find all records with a min install not exceeding 50, that have a rating. 
